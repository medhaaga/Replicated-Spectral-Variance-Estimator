...
perspective <- function(A, B, C, pts)
{
  x = seq(-2, 2*C, length= pts)
  y = x
  f = function(x, y) {
    r = exp(-.5*(A*(x*y)^2 + x*x + y*y - 2*B*x*y -2*C*x - 2*C*y))
    return(r)
  }
  z = outer(x, y, f)
  z[is.na(z)] = 1
...
markov.chain <- function(A,B,C,nsim,start){
  x.samp <- rep(0,nsim)
  y.samp <- rep(0,nsim)
  ...
  for (i in 2:nsim){
    x.samp[i] <- rnorm(1,mean = (B*y.samp[i-1] + C)/(1 + A*y.samp[i-1]^2), sd = sqrt(1/(1 + A*y.samp[i-1]^2)))
    y.samp[i] <- rnorm(1,mean = (B*x.samp[i] + C)/(1 + A*x.samp[i]^2), sd = sqrt(1/(1 + A*x.samp[i]^2)))
  }
...
true.mean <- function(A,B,C){
  integrand <- function(x) {exp(-.5*(A*(x[1]*x[2])^2 + x[1]*x[1] + x[2]*x[2] -2*B*x[1]*x[2] -2*C*x[1] - 2*C*x[2]))}
  normalise_const.a <- adaptIntegrate(integrand, lowerLimit = c(-Inf, -Inf), upperLimit = c(Inf, Inf), tol = 1e-10)
  mean_fn <- function(x) {x[1]*exp(-.5*(A*(x[1]*x[2])^2 + x[1]*x[1] + x[2]*x[2]  -2*B*x[1]*x[2] - 2*C*x[1] - 2*C*x[2]))/normalise_const.a$integral}
