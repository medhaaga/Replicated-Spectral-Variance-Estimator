...
create.output1 <- function(A,B,C,m,check.pts,freq,c.prob){
  ...
  for (i in 1:length(check.pts)){
    ...
    for (j in 1:freq){
      ...
      for (k in 1:m){
        ...
        #m <- lm(chain[,,k] ~ 1)
        #b[k] <- bwAndrews(m, kernel = "Tukey-Hanning", approx = "AR(1)", prewhite = 0)
      }
    ...
    #save(asv.samp,rsv.samp,asv.coverage,rsv.coverage, file = paste(paste("Out/tukey/", A, B, C, "/out", m,nsim,A,B,C, sep = "_"),".Rdata", sep = ""))
  }
...
##################################################
convergence1 <- function(min, max, A, B, C, m, rep=100){
  ...
  #save(asv.samp,rsv.samp, ess.asv.samp, ess.rsv.samp, file = paste(paste("Out/tukey/", A, B, C, "/conv_data", m, min, max, A, B, C, sep = "_"), ".Rdata", sep = ""))
...
create.output2 <- function(A,B,C,m,check.pts,freq,c.prob){
...
#function for storing convergence plots data
##################################################
convergence2 <- function(min, max, A, B, C, m, rep=100){
...
print("Carrying out 1000 repititions for each value of nsim in check.pts")
create.output2(A, B, C, m, check.pts, freq, c.prob)
