...
for(i in 1:m)
  start[i,] <- rmvnorm(n=1, mean = rep(0,p), sigma = diag(10*sqrt(diag(target))))
...
save(mc.chain.list, true.acf, file = "Out/var-fifty_chains.Rdata")
#################################################################################
...
for (n in 1:2)
{
  ...
  for (t in (1:floor(m/2)))
  {
    ...
    global.acf <- globalACF(x, type = "correlation", component = 1, lag.max = lag.max, chains = 0, plot = FALSE, avg = FALSE)[[1]]
    local.acf <- globalACF(x, type = "correlation", component = 1, mean = "local", lag.max = lag.max, chains = 0, plot = FALSE, avg = FALSE)[[1]]
    
    lacf_twolags[1,t] <- local.acf[[1]][1+lag1]
    lacf_twolags[2,t] <- local.acf[[1]][1+lag2]
    gacf_twolags[1,t] <- global.acf[[1]][1+lag1]
    gacf_twolags[2,t] <- global.acf[[1]][1+lag2]
    
  }
  
  pdf(file = paste("Out/ACFvsm_", ncrop, ".pdf", sep = ""), height=5, width=5)
  plot(seq(2, 50, 2), lacf_twolags[1,], type = "l", ylim = c(0,1), col = "blue", lwd=2, ylab = "ÄCF", xlab = "m")
  lines(seq(2, 50, 2), lacf_twolags[2,], col = "blue", lwd=2, lty=2)
  lines(seq(2, 50, 2), gacf_twolags[1,], col = "orange", lwd=2)
  lines(seq(2, 50, 2), gacf_twolags[2,], col = "orange", lwd=2, lty=2)
