...
lines(seq(-lag.max, lag.max), true.acf[1,1,], col = "red")
plot(acf.list[[2]][[1]], main = paste("Locally centered ACF plot for component - 5"), xlim = range(0, lag.max), ylim = c(min(min(acf.list[[2]][[1]]$acf), min(true.acf[2,2,])), max(max(acf.list[[2]][[1]]$acf), max(true.acf[2,2,]))))
lines(seq(-lag.max, lag.max), true.acf[2,2,], col = "red")
plot(acf.list[[2]][[2]], main = paste("Globally centered ACF plot for component - 5"), xlim = range(0, lag.max), ylim = c(min(min(acf.list[[2]][[2]]$acf), min(true.acf[2,2,])), max(max(acf.list[[2]][[2]]$acf), max(true.acf[2,2,]))))
lines(seq(-lag.max, lag.max), true.acf[2,2,], col = "red")

plot(ccf.list[[1]], main = "Locally centered CCF plot between 1 and 5", xlim = c(-lag.max, lag.max), ylim = range(ccf.list[[1]]$acf))
lines(seq(-lag.max, lag.max), true.acf[1,2,], col = "red")
