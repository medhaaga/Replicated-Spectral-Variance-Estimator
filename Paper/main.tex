...
\]
{\color{blue} Add more about autocorrelation. Refer to Figure 2 above and the impacts of the correlation methods. }
...
{\color{blue} Don't make this an assumption. Say this explicitly in Theorem 2. }

\begin{ass} \label{ass:sve_consis} We assume that all the conditions given by \cite{vats:fleg:jon:2018} for strong consistency of the spectral variance estimator hold true. As a consequence, $\hat{\Sigma}_{S,s} \xrightarrow{a.s.} \Sigma$ as $n \to \infty$ for all Markov chains $s$.
% \[
...
{\color{blue}: Make the following an assumption. Is $2\pi$ needed? Is $W_n$ always finite? }
...
{\color{blue} Under what assumptions?}
...
{\color{blue} Don't make this a separate assumption. Include in the statement of the theorem}
...
\end{align*}
%
Additionally, the locally-centered autocovariance exhibits the following expectation {\color{blue} + add small order term. Confirm with Priestly. }
 \begin{equation} \label{eq:priestly}
     \mathbb{E}[\hat{\Gamma}(k)] = \left(1- \dfrac{\abs{k}}{n}\right)\left(\Gamma(k) - \Var{(\bar{X})}
 \right)\,.
 \end{equation}
