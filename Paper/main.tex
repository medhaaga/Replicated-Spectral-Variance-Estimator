...
SV estimators are constructed as weighted and truncated sums of estimated lag covariances. Let $w: \mathbb{R} \to [-z,z]$ be a lag window function and let $b_n \in \mathbb{N}$ be a truncation point. The SV estimator of $\Sigma$ from a single chain is
%
{\color{blue} DV: limits should be $-n +1$ and $n - 1$.}
...
% \end{equation}
{\color{blue} DV: There are some issues.
\begin{itemize}
  \item $|w_n| < z$ or $|w_n| < c$?
  \item Need new notation for $\Gamma(k)$ since $\Gamma(k)$ is for the $X$ and we need a new notation for $Y$. This change needs to be made for this whole section and the appendix!
\end{itemize}
}
...
%
\begin{align*}
    \hat{\Sigma}_{G} &= \sum_{k=-b_n+1}^{b_n-1}w\left(\dfrac{k}{b_n}\right)\hat{\Gamma}_{G}(k)\,.
\end{align*}
% and the combined G-SVE is
% \[
% \hat{\Sigma}_{G} =  \dfrac{1}{m}\sum_{s=1}^{m}\hat{\Sigma}_{G,s}\,.
% \]
For $\hat{\Sigma}_{G}$ to accurately estimate $\Sigma$, conditions need to be imposed on the weight function, $w_n$.

{\color{blue}DV: Did we discuss that $W_n$ should be define as an infinite sum and should be $W$?}

%

\begin{ass}
\label{ass:lag_window}
The lag window $w(x)$ is a continuous, even function with $w(0)=1, \abs{w(x)} < c$, $\int_{-\infty}^{\infty}w^2(x)dx < \infty$, and $W_n < \infty$ where
\[
...
Assumption~\ref{ass:lag_window} is standard \citep[see]{ande:1971} and is satisfied by most lag windows. We will use the popular Bartlett lag window in our simulations. {\color{blue}DV: write the Bartlett lag window}. 
...
\begin{theorem}[\cite{kuel:1976,vats:fleg:jon:2018}]
  \label{thm:kuelbs}
Let $\E_F\|Y_{11}\|^{2+ \delta} < \infty$ for $\delta > 0$ and let $P$ be a polynomially ergodic Markov chain of order $\xi > (q + 1 + \epsilon)/(1 + 2/\delta)$ for $q \geq 1$. Then there exists a $p \times p$ lower triangular matrix $L$ such that $LL^T = \Sigma$, a non-negative function $\psi(n) = n^{1/2 - \lambda}$ for some $\lambda > 0$, a finite random variable $D$, and a sufficiently rich probability space $\Omega$ such that for almost all $w \in \Omega$ such that for all $n > n_0$, with probability 1,
\[
...
{\color{blue} Move these two sub-assumptions into the theorem 4 below.}
...
Let assumptions of Theorem~\ref{thm:kuelbs} hold with $q$ such that {\color{blue}......} Let the Assumption~\ref{ass:bias} hold. Then
\[
...
\begin{theorem} \label{th:G-SVE_variance}
 Let assumptions of Theorem~\ref{thm:kuelbs} hold and let $\E[D^4] < \infty$  and  $\E \|Y_{11}\|^4 < \infty$, then $b_n^{-1}{n}\Var \left(\hat{\Sigma}_{G}^{ij} \right) = [\Sigma_{ii}\Sigma_{jj} + \Sigma_{ij}^2]\int_{-\infty}^{\infty}w(x)^2dx  + o(1)$.
\end{theorem}

{\color{blue} I have not looked at it after here. }
