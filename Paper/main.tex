...
For a diagonal element of $\Gamma$,
\begin{align*}
& \E \left[ \hat{\Gamma}_{RAC,s}^{ii}\right]\\
& = \mathbb{E}\left[\hat{\Gamma}_{s}^{ii}(k)\right] - {\color{red}\dfrac{2|k|}{n}\dfrac{m-1}{mn}\left(\sum_{h=0}^{n-1}\Gamma^{ii}(h) - n \Var(\bar{X}_1)^{ii}\right)} + \left(1- \dfrac{|k|}{n}\right)\left(1-\dfrac{1}{m}\right)\Var(\bar{X}_1)^{ii}\,.
\end{align*}
...
%
{\color{red} Complete the algebra here}
\begin{align*}
    g_1(n) &= (4+C_1)\dfrac{b_n \psi^2(n)}{n^2} - 4\dfrac{\psi^2(n)}{n^2} \to 0\\
    g_2(n) &= 2\sqrt{2}\|L\|p^{1/2}(1+\epsilon)\left[(4+C_1)\dfrac{b_n\psi(n)\sqrt{n\log \log n}}{n^2} - 4\dfrac{\psi(n)\sqrt{n\log \log n}}{n^2}\right] \to 0\\
    g_3(n) &= \|L\|^2 p (1+\epsilon)^2\left[(4+C_1)\dfrac{b_n \log\log n}{n} - 4 \dfrac{\log \log n}{n}\right] \to 0\,.
...
Similar to the above proof, second part of $N_1$ can also be written as a summation of $k$ terms and the proof follows exactly as the first part. Now we prove that $N_2 \xrightarrow{a.s.} 0 \textrm{ as } n \to \infty$. {\color{red} Remove the following proof maybe because repetitive}
%
\begin{align*}
    |N_2|&= \left|\dfrac{1}{m} \sum_{s=1}^{m} \left(\bar{X}_s - \mu \right)_i \left(\bar{X}_s - \mu \right)_j\sum_{k=-b_n+1}^{b_n-1}w\left(\dfrac{k}{b_n}\right)\left(1 - \dfrac{\abs{k}}{b_n}\right) \right|\\
...
\end{equation}
%
\eqref{eq:PASVE_term-2} is also $\mathcal{O}\left({b_n \log \log n}/n\right)$, and hence converges to 0 as $n \to \infty$.
\end{proof}
