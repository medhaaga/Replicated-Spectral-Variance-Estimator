...
\onehalfspacing
\begin{abstract}
Autocovariances are the fundamental quantity of interest in many features of Markov chain Monte Carlo (MCMC) simulations with autcorrelation function (ACF) plots being often used as a visual tool to ascertain the performance of a Markov chain. Unfortunately, for slow mixing Markov chains, the empirical autocovariance can highly underestimate the truth. For multiple chain MCMC sampling,  we propose a globally-centered estimator of autocovariance function (G-ACvF) that pools information from all Markov chains. G-ACvF is also shown to correct the negative bias of single chain empirical autocovariance. We show the impact of this improved estimator in three aspects: (1) ACF plots, (2) estimates of the Monte Carlo asymptotic covariance matrix, and (3) estimates of the effective sample size. We also establish the strong consistency of our improved asymptotic covariance estimator under weak conditions, and obtain its large sample bias and variance. 
\end{abstract}
...
G-SV  &    0.956 & 0.937 & .924 & .945 & .952 \\ \hline
    \end{tabular}
    \caption{VAR(1). Coverage probabilities at $95 \%$ nominal level using A-SV and G-SV estimator.}
...
\end{align*}
where $g_1(n), g_2(n), g_3(n)$ are defined such that as $n \to \infty$,  $g_1(n), g_2(n), g_3(n) \to 0$.
%\begin{align*}
%   g_1(n) &= (4c+C)\dfrac{b_n \psi^2(n)}{n^2} - 4c\dfrac{\psi^2(n)}{n^2} \to 0\\
%    g_2(n) &= 2\sqrt{2}\|L\|p^{1/2}(1+\epsilon)\left[(4c+C)\dfrac{b_n\psi(n)\sqrt{n\log \log n}}{n^2} - 4c\dfrac{\psi(n)\sqrt{n\log \log n}}{n^2}\right] \to 0\\
%    g_3(n) &= \|L\|^2 p (1+\epsilon)^2\left[(4c+C)\dfrac{b_n \log\log n}{n} - 4c \dfrac{\log \log n}{n}\right] \to 0\,.
...
\begin{figure}[htbp]
    ...
    \caption{Change point: ACF plots using local-centering (left) and global-centering (right) for $m=2$ parallel Markov chains. The individual chains are shown through dashed lines and average over $m$ chains is the solid line. The chain lengths are $n = 1000$ (top row) and $n = 10000$ (bottom row).}
    \label{fig:poisson-acf}
\end{figure}

Figure~\ref{fig:poisson-acf} demonstrates a striking advantage of G-ACF against locally-centered ACF in estimating the autocorrelations. For a chain length of $n=1000$, locally-centered ACF gives a false sense of security about the Markov chains whereas in reality, the process is highgly correlated. G-ACF gives a more realistic and accurate estimation of autocorrelations in almost ten times lesser samples per chain.
...
We believe that the students from different races experience different socio-political conditions which causes them to engage in a "selective networking" which might cause formation of clusters in the social network. We sample two parallel Markov chains starting from two students belonging to different races and study its impacts on the average features of their immediate social group, especially their grades. Due to cluster formation in the network, a Markov chain will land on students within a cluster more often than on students outside it. As a consequence, the sample averages for the two chains will differ for a relatively shorter chain length. 
