...
The lag window $w(x)$ is continuous at $0$ at all but finite number of points, is a bounded and even function with $w(0)=1$, \; $\int_{-\infty}^{\infty}w^2(x)dx < \infty$, and $\sum_{k = -\infty}^{\infty} w(k/b_n) < \infty$.
\end{ass}
